...
sub run {
    ...
    my $total_orders = $schema->resultset('Orders')->count;
    ok($total_orders);
    ...
    ## Destroy a single order via instance
    {
        ...
        my $remaining_orders = $schema->resultset('Orders')->count;
...
## pass in storage instead
{
    ...
    Handel::Test::RDBO::Order->destroy({
...
## don't unset self if no result is returned
{
    ...
    my $order = Handel::Test::RDBO::Order->search({id => '22222222-2222-2222-2222-222222222222'}, {storage => $storage})->first;
    ok($order);
